 ...
 * 
 * Usage:
 *   node scripts/add-test-user-to-all-communities.js
 */
...
async function addTestUserToAllCommunities() {
  ...
  try {
    ...
    const existingCommunityIds = new Set(
      (existingMemberships || []).map((m) => m.community_id)
    );
    ...
    const membershipsToInsert = communitiesToAdd.map((community) => ({
      user_id: testUser.id,
    ...
    communities.forEach((community, index) => {
      const isNew = communitiesToAdd.some((c) => c.id === community.id);
      const marker = isNew ? '🆕' : '✓';
    ...
    // Check if user can perform interactions
    // Members can:
    // - React to posts/comments (via RLS policies)
    // - Comment on posts (via RLS policies)
    // - Create posts (via RLS policies)
