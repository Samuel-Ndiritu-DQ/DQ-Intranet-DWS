...
CREATE INDEX idx_news_news_type ON public.news(news_type);
CREATE INDEX idx_news_tags ON public.news USING GIN(tags);

CREATE INDEX idx_jobs_posted_on ON public.jobs(posted_on DESC);
...
CREATE INDEX idx_jobs_sfia_level ON public.jobs(sfia_level);
...
CREATE POLICY "Allow authenticated update to news" ON public.news
  FOR UPDATE USING (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated insert to jobs" ON public.jobs
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Allow authenticated update to jobs" ON public.jobs
  FOR UPDATE USING (auth.role() = 'authenticated');
